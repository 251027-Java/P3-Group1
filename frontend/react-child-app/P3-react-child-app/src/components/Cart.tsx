...
const Cart = () => {
  ...
  const handleCheckout = async () => {
    // Simulated checkout for token packages. In production, integrate with Stripe or backend.
    alert('Simulated checkout — proceed to backend purchase flow to credit tokens.');
  };
  ...
  return (
    <div className={`fixed inset-0 z-[100] transition-opacity duration-300 ${isCartOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
      ...
      <div className={`absolute right-0 top-0 h-full w-full max-w-md bg-[#0f0f0f] border-l border-white/10 shadow-2xl transition-transform duration-300 transform ${isCartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col h-full">
          <div className="p-6 border-b border-white/10 flex justify-between items-center">
            <h2 className="text-xl font-bold uppercase tracking-tighter italic">Your Manifest</h2>
            <button onClick={closeCart} className="text-gray-500 hover:text-white">✕</button>
          </div>
              ...
              items.map((item, i) => (
                <div key={i} className="flex gap-4 group">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="mt-2 text-[#822C2C] font-mono text-sm">{item.tokens ? item.tokens + ' tokens' : `$${item.price}`}</p>
                  </div>
                  <button onClick={() => removeFromCart(i)} className="text-gray-700 hover:text-red-500 text-xs">Remove</button>
                </div>
              ))
            )}
          </div>
          ...
          <div className="p-6 bg-[#151515] border-t border-white/10">
            ...
            <div className="flex gap-2">
              <button 
                ...
                className="flex-1 py-4 bg-[#822C2C] hover:bg-[#a13737] disabled:bg-gray-800 disabled:text-gray-600 font-bold uppercase tracking-[0.2em] rounded-sm transition-all shadow-lg shadow-red-900/20"
              >
                Purchase Tokens
              </button>
              <button onClick={() => clearCart()} className="py-4 px-6 bg-white/5">Clear</button>
            </div>
